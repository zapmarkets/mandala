...
contract MandalaStETHTreasury is IMandalaStETHTreasury, ReentrancyGuard {
    ...
    constructor(address policyAddress, address registryAddress, address wstETHAddress) {
    ...
    function fundTask(address taskAddress, uint256 amount) external nonReentrant {
    ...
    /// @inheritdoc IMandalaStETHTreasury
    function claimReward(address taskAddress) external nonReentrant {
    ...
    /// @inheritdoc IMandalaStETHTreasury
    function refund(address taskAddress) external nonReentrant {
        TaskDeposit storage dep = _deposits[taskAddress];
        if (dep.wstETHAmount == 0) revert TaskNotFunded();
        if (dep.claimed) revert AlreadyClaimed();
